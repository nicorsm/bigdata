...
val data = spark.read.format("csv").option("header", "true").load("hdfs://localhost:8020/user/cloudera/dataset/2007.csv")
...
val airports = spark.read.format("csv").option("header", "true").load("hdfs://localhost:8020/user/cloudera/dataset/airports.csv").select($"iata",$"airport")
